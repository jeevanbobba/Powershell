...
$TOLIST = "Surapaneni.Venu@dol.gov","Wright.Brian.D@dol.gov"
$SBforDC = {
Add-PSSnapin ci*
...
$WMIDiskInfo = Get-CimInstance -ClassName Win32_Volume -Property Capacity,FreeSpace,DriveLetter | Where {$_.DriveLetter -eq $env:SystemDrive} | Select Capacity,FreeSpace,DriveLetter
[pscustomobject]@{
sfstatus = $(Get-STFServerGroup).ClusterMembers | ft -AutoSize
sfdetails = Get-STFWebReceiverService | select FriendlyName,VirtualPath | ft -AutoSize
days = (New-TimeSpan -Start $oswmi.lastbootuptime -End $(get-date)).Days
cfreespace = [math]::Round(($WMIDiskInfo.FreeSpace * 100 / $WMIDiskInfo.Capacity))
...
StorefrontStatus = ($extraSFdetails | ?{$_.PScomputerName -eq $XD.PSComputerName}).sfstatus | Out-String
StorefrontSites = ($extraSFdetails | ?{$_.PScomputerName -eq $XD.PSComputerName}).sfdetails | Out-String
...
#Director DATA
$DirectorInfo = icm -ComputerName  $directorServer -Credential $cred1 -ScriptBlock {
$oswmi = Get-CimInstance -ClassName win32_operatingsystem
$WMIDiskInfo = Get-CimInstance -ClassName Win32_Volume -Property Capacity,FreeSpace,DriveLetter | Where {$_.DriveLetter -match ":"} | Select Capacity,FreeSpace,DriveLetter
[pscustomobject]@{
'Uptime(in Days)' = (New-TimeSpan -Start $oswmi.lastbootuptime -End $(get-date)).Days
...
vmtools = if((Get-Service -Name "VMTOOLS" -ErrorAction SilentlyContinue).Status -notmatch "running"){"Not Running"}else {"Running"}
OSversion = $oswmi.Version
DirDetails = (Get-WmiObject -Class Win32_Product | where Name -match "Director"| select @{l="a";e={$_.Name,$_.version}}).a
'AvgCPU(%)'= [math]::Round((GET-COUNTER -Counter "\Processor(_Total)\% Processor Time" -SampleInterval 3 -MaxSamples 8 |select -ExpandProperty countersamples | select -ExpandProperty cookedvalue | Measure-Object -Average).average)
'AvgMem(%)' = [math]::Round((GET-COUNTER -Counter "\Memory\% Committed Bytes In Use" -SampleInterval 3 -MaxSamples 8 |select -ExpandProperty countersamples | select -ExpandProperty cookedvalue | Measure-Object -Average).average)
'EPO(DATE)' = (Get-ItemProperty 'HKLM:\SOFTWARE\WOW6432Node\McAfee\AVEngine' -Name 'AVDatDate').AVDatDate
'EPO(DAT)' = (Get-ItemProperty 'HKLM:\SOFTWARE\WOW6432Node\McAfee\AVEngine' -Name 'AVDatVersion').AVDatVersion
...
$DirectorInfo = $DirectorInfo | select @{l="Director";e={$directorServer}},* -ExcludeProperty PScomputerName,RunspaceId
...
$XDprofiledata = $reportdata | ?{$_.USer -notmatch 'User Not Found in AD'} | Sort-Object -Property 'Size(MB)' -Descending 
...
foreach ($reportline in $SFResults)
    {
        ...
        $htmltablerow += "<td>$($reportline.StoreFrontStatus)</td>"
...
foreach ($reportline in $BrokerResults)
    {
      ...
      if ($($reportline.MemUsg) -gt 70)
        ...
        $htmltablerow += "<td>$($reportline.ConnectedIsers)</td>"
...
$dirdataGrag = $Directordata | ConvertTo-Html -As Table -Fragment -PreContent "<h2>Citrix Director </h2>"  -PostContent "<br></br>"   | Out-String
$XDProfileFrag = $XDprofiledata | ConvertTo-Html -As Table -Fragment -PreContent "<h2>Citrix Profile Report </h2>"  -PostContent "<br></br>"   | Out-String
$XDProfileInactiveFrag = $inactiveusers | ConvertTo-Html -As Table -Fragment -PreContent "<h2>Citrix Director </h2>"  -PostContent "<br></br>"   | Out-String
...
$body = ConvertTo-HTML  -PreContent $finalreport,$catfrag,$Licfrag,$DBFrag,$dirdataGrag,$XDProfileFrag,$XDProfileInactiveFrag  -Title "<h2>Citrix Daily Checks $(get-date -Format D) </h2>"  -PostContent "<p class='footer'>Report generated from $env:COMPUTERNAME on $(get-date) .Report is run using $($cred1.username)</p>"
