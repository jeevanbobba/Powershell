...
$reportemailsubject = "Domain Controllers Health Report"
######################################################################
############### Email Configuration ##################################
    $Subject = "Domain -$reportemailsubject - $now"
    $SmtpServer = "forwarder.dexko.online"
    $from = "jeevan.bobba@dextergroup.com"
    $To = "jeevan.bobba@dextergroup.com"
    #$port = 25
    $priority = "Normal"
#...................................
# Functions
...
foreach ($domain in $allDomains){
    ...
    foreach ($domainController in $allDomainControllers){
       # $Ipaddress = Get-WmiObject -Class Win32_NetworkAdapterConfiguration -ComputerName $domainController.HostName |? {$_.servicename -eq "vmxnet3ndis6"}
        ...
        $thisDomainController | Add-Member NoteProperty -name "Uptime (Days)" -Value $null
        ...
        $thisDomainController."Uptime (Days)" = Get-DomainControllerUpTime $domainController.HostName
    ...
    $htmlhead="<html>
                ...
                <h3 align=""left"">Generated From: $env:computername  $reportime</h3>"
    ...
    foreach ($reportline in $allTestedDomainControllers)
    {
      ...
      if (Test-Path variable:fsmoRoleHTML)
        ...
        if ($($reportline."uptime (Days)") -eq "WMI Failure")
        {
            $htmltablerow += "<td class=""warn"">Could not test server uptime.</td>"        
        }
        elseif ($($reportline."Uptime (Days)") -eq $string17)
        {
            $htmltablerow += "<td class=""warn"">$string17</td>"
        }
        else
        {
            $hours = [int]$($reportline."Uptime (Days)")
            if ($hours -le 24)
            {
...
#>
    ...
    if ($SendEmail)
    {
      ...
      Send-MailMessage -SmtpServer $SmtpServer -From $from -To $To  -Subject $Subject -Bodyashtml $htmlreport -Priority $priority #-Port $port
    } 
