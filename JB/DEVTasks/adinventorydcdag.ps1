...
$reportime = Get-Date -Format 'MM-dd-yyyy HH:mmtt'
$reportemailsubject = "Domain Controller Health Report"
######################################################################
############### Email Configuration ##################################
    $Subject = "COE-DEV-$reportemailsubject - $now"
    $SmtpServer = "smtp.dev.dol.gov" # "smtp.dev.dol.gov"  #"smtp.dev-ent.DEV-DIR.LABOR.GOV" 
    $from = "bobba.jeevan@dev.dol.gov" # "Bobba.Jeevan@DOL.gov" 
    $To = "bobba.jeevan@dol.gov"
    $Bcc = "Bobba.Jeevan@DOL.gov"#,"Behzad.Ellie@dol.gov","Surapaneni.Venu@dol.gov","Simo.Noumbisie@dol.gov","Quintanilla.Raul.H@DOL.gov","Onadeko.Eddie@dol.gov","Manikya.Sandan@dol.gov","Katneni.Krishna.C@dol.gov","WatsonIII.George.A@DOL.GOV"
    #$Bcc = "Bobba.Jeevan@DOL.gov" #,"Manikya.Sandan@dol.gov","Katneni.Krishna.C@dol.gov"
    $port = 25
    $priority = "Normal"
#...................................
# Functions
...
#This function checks the IP address of the Domain Controller machine
Function Get-DomainControllerIPaddress ($domainNameInput){
Write-Verbose "..running function Get-DomainControllerIpaddress"
$thisIPAddress = (Get-WmiObject -Class Win32_NetworkAdapterConfiguration -ComputerName $domainNameInput -ErrorAction SilentlyContinue |? {$_.servicename -eq "vmxnet3ndis6"}).Ipaddress
...
$DCDiagTest = (Dcdiag.exe /s:$domainNameInput /test:services /test:KnowsOfRoleHolders) -split ('[\r\n]')
...
#This is HTML resport file Name file creation
if (!($domainName)){
Write-Host "..no domain specified, using all domains in forest" -ForegroundColor Yellow
$allDomains = Get-AllDomains
$reportFileName = " "
$reportFileName = $reportime +'_'+'forest_health_report_' + (Get-ADForest).name +'.html'
}
...
$reportFileName = $reportime + '_' +'dc_health_report_' + $domainName + '.html'
}
...
foreach ($domain in $allDomains){
    ...
    #$allTestedDomainControllers = $null

    foreach ($domainController in $allDomainControllers){
       # $Ipaddress = Get-WmiObject -Class Win32_NetworkAdapterConfiguration -ComputerName $domainController.HostName |? {$_.servicename -eq "vmxnet3ndis6"}
        ...
        #$thisDomainController | Add-Member NoteProperty -name DNSIP -Value $null
        ...
        $OFS = "`r`n"
        $thisDomainController.Domain = ($domainController.Domain).ToLower()
        $thisDomainController.Server = ($domainController.HostName).ToLower()
        $thisDomainController.IPAddress = (Get-DomainControllerIPaddress $domainController.hostname)
    ...
    $htmlhead="<html>
                ...
                <body>
                <h1 align=""Center"">Domain Controller Health Check Report</h1>
                <h3 align=""left"">Generated: $reportime</h3>"
                   
    #Domain Controller Health Report Table Header
    $htmltableheader = "<h3>COE-DEV Domain Controller Health Summary</h3>
    ...
    #Domain Controller Health Report Table    <th>DIT Free Space (%)</th>  <th>DCDIAG: Advertising</th>
...
#>
    ...
    if ($SendEmail)
    {
      ...
      #Send-MailMessage -From "$from" -To "$to" -Subject "$Subject" -Body $htmlreport -BodyAsHtml -Encoding ([System.Text.Encoding]::UTF8) -SmtpServer "$SmtpServer"
      Send-MailMessage -SmtpServer $SmtpServer -From $from -To $To -Bcc $Bcc -Subject $Subject -Bodyashtml $htmlreport -Port $port -Priority $priority
    } 
    #$htmlreport| convertto-html c:\temp\report.html
   #Send-MailMessage -SmtpServer $SmtpServer -From $from -To $to -Subject $Subject -Bodyashtml $htmlreport -Port $port -Priority $priority

   # Send-MailMessage -SmtpServer $smtpServer -From $emailFrom -To $emailTo -Bcc $emailBcc -Subject $subject -BodyAsHtml $body -Port $port  -Priority $priority