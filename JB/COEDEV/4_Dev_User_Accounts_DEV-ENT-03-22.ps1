...
#       Reviewer:    
#       Date: 03/21/2022
#       Satus: Created User acconts in DEV-ENT
#       Update: 03/21/2022 addIn UPN
#               00/00/2021  Change domain to DEV-ENT
#               10/03/2021 Changing Home directory to \\Coe_dev_nas.dev-ent.dev-dir.labor.gov\HomeDrives$                
#               Added Advertising
...
if ($env:USERDNSDOMAIN -like "*dev-dir*")
{
	...
	function Add-COEDEVuser()
	{
		param (
            ...
            [string]$DisplayName,
			[string]$hdrivepath = '\\Coe_dev_nas.dev-ent.dev-dir.labor.gov\HomeDrives$',
        ...
        {
			#added Firstname and lastname prod emailid, displayname, serPrincipalName
            New-aduser -SamAccountName $user -Name $name -GivenName $firstname -Surname $lastname -Server $DomainName -Employeenumber $Prodemail -Enabled:$true -AccountPassword $passwd -Path $DomainOU -UserPrincipalName $Prodemail 
		}
		finally
		{
			New-Item -Path "$hdrivepath\$user" -ItemType Directory -ErrorAction SilentlyContinue -Force
			Set-ADUser -Identity $user  -HomeDirectory $hdrivepath\$user -HomeDrive 'H:' -Description $description -Server $DomainName -DisplayName $DisplayName -UserPrincipalName $Prodemail -Employeenumber $Prodemail
			...
			if (!((get-aduser $user -Server $DomainName -Properties Enabled).Enabled))
			{
				...
				#added firstname and lastname prod emailid displayname User principal name
                Set-ADUser -Identity $user -Name $name -GivenName $firstname -Surname $lastname -DisplayName $DisplayName -HomeDirectory $hdrivepath\$user -HomeDrive 'H:' -Description $description -ChangePasswordAtLogon:$true -Enabled:$true -Server $DomainName -Employeenumber $Prodemail -UserPrincipalName $Prodemail 
                Move-ADObject (Get-Aduser $user -Server $DomainName)  -Server $DomainName -TargetPath $DomainOU -Confirm:$false
			}
		}
	}
	foreach ($Data in $VMCSVDATA)
	{
       ...
       if ($Agency -like "OWCP")
		...
		Add-COEDEVUSER -DomainName $DomainName -OUPath $DomainOU -AgencyWKSAdminGroup $DomainAdminGroup -User $user -name $name -firstname $firstname -lastname $lastname -Prodemail $Prodemail -DisplayName $DisplayName
	    } 
  }